...
FMIStatus FMI3SetValues(
 ...
 FMIStatus FMIGetContinuousStateValueReferences(
     ...
     fmi2Real* pre_x = (fmi2Real*)calloc(nx, sizeof(fmi2Real));
     fmi2Real* x = (fmi2Real*)calloc(nx, sizeof(fmi2Real));

     if (!pre_x || !x) return FMIError;

     for (size_t i = 0; i < nx; i++) x[i] = i;

     CALL(FMI2GetContinuousStates(instance, pre_x, nx));

     CALL(FMI2SetContinuousStates(instance, x, 2));

     for (size_t i = 0; i < nx; i++) {
         const FMIModelVariable* derivative = modelDescription->derivatives[i].modelVariable;
         const FMIModelVariable* state = derivative->derivative;
         fmi2Real value;
         CALL(FMI2GetReal(instance, &state->valueReference, 1, &value));
         const size_t j = (size_t)value;
         instance->continuousStateValueReferences[j] = state->valueReference;
         instance->derivativeValueReferences[j] = derivative->valueReference;
     }

     CALL(FMI2SetContinuousStates(instance, pre_x, nx));
